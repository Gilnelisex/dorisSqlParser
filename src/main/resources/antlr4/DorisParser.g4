...
supportedDropStatement
    ...
    | DROP DATABASE (IF EXISTS)? dbname=identifier (FORCE)?                    #dropDatabase
    ;
...
dropColumnDefs
    : cols+=dropColumnDef (COMMA cols+=dropColumnDef)*
    ;
...
dropColumnDef
    : DROP COLUMN colName=identifier (FROM rollupName=identifier)?
    ;
...
addPartitionsDef
    : partitions+=addPartitionDef (COMMA partitions+=addPartitionDef)*
    ;

dropPartitionsDef
    : partitions+=dropPartitionDef (COMMA partitions+=dropPartitionDef)*
    ;

addPartitionDef
    : ADD partitionDef (DISTRIBUTED BY (HASH hashKeys=identifierList | RANDOM) (BUCKETS (INTEGER_VALUE | autoBucket=AUTO))?)?
    ;

dropPartitionDef
    : DROP partitionDef (FORCE)?
    ;
